...
class ReplayBuffer():
    ...
    def data_augmentation(trajectory):
        ...
        for s_a_p_w in trajectory:
            ...
            t1.append([state, np.reshape(action, newshape=(action.shape[0], action.shape[1])), np.reshape(policy, newshape=(policy.shape[0] * policy.shape[1])), winner])
            t2.append([state_flip_1, np.reshape(action_flip_1, newshape=(action_flip_1.shape[0], action_flip_1.shape[1])), np.reshape(policy_flip_1, newshape=(policy_flip_1.shape[0] * policy_flip_1.shape[1])), winner_flip_1])
            t3.append([state_rot90, np.reshape(action_rot90, newshape=(action_rot90.shape[0], action_rot90.shape[1])), np.reshape(policy_rot90, newshape=(policy_rot90.shape[0] * policy_rot90.shape[1])), winner_rot90])
            t4.append([state_flip_2, np.reshape(action_flip_2, newshape=(action_flip_2.shape[0], action_flip_2.shape[1])), np.reshape(policy_flip_2, newshape=(policy_flip_2.shape[0] * policy_flip_2.shape[1])), winner_flip_2])
            t5.append([state_rot180, np.reshape(action_rot180, newshape=(action_rot180.shape[0], action_rot180.shape[1])), np.reshape(policy_rot180, newshape=(policy_rot180.shape[0] * policy_rot180.shape[1])), winner_rot180])
            t6.append([state_flip_3, np.reshape(action_flip_3, newshape=(action_flip_3.shape[0], action_flip_3.shape[1])), np.reshape(policy_flip_3, newshape=(policy_flip_3.shape[0] * policy_flip_3.shape[1])), winner_flip_3])
            t7.append([state_rot270, np.reshape(action_rot270, newshape=(action_rot270.shape[0], action_rot270.shape[1])), np.reshape(policy_rot270, newshape=(policy_rot270.shape[0] * policy_rot270.shape[1])), winner_rot270])
