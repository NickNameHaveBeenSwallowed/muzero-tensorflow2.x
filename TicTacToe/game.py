...
class TicTacToe(gym.Env):
    ...
    def done(self):
        ...
        filter0 = np.array([1, 1, 1])
        filter1 = np.array([[1], [1], [1]])
        filter2 = np.eye(3)
        filter3 = np.eye(3)[::-1]
        done = check(filter0, self.board[0], self.num_chess, 3, 1) or done
        done = check(filter0, self.board[1], self.num_chess, 3, 1) or done
        done = check(filter1, self.board[0], self.num_chess, 1, 3) or done
        done = check(filter1, self.board[1], self.num_chess, 1, 3) or done
        done = check(filter2, self.board[0], self.num_chess, 3, 3) or done
        done = check(filter2, self.board[1], self.num_chess, 3, 3) or done
        done = check(filter3, self.board[0], self.num_chess, 3, 3) or done
        done = check(filter3, self.board[1], self.num_chess, 3, 3) or done
        return done
